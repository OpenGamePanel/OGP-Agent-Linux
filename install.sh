...
init_dir=${init_dir%/}
echo 'export AGENTDIRECT=${agent_home}' >> ~/.profile # Make Permanently Environment for Agent Directory , Usage : Game Addons and etc...
echo 'export AGENTUSERNM=${username}' >> ~/.profile # Make Permanently Environment for Agent UserName , Usage : Game Addons and etc...
echo 'export AGENTPW=$(cat "$AGENTDIRECT/Cfg/Config.pm" | grep -o "sudo_password.*" | grep -ow "[^sudo_password( \)*=>( \)*].*" | grep -o "[^'].*[^',]")' >> ~/.profile #Usage : Sudo Permission For Addons and etc... , Example : sudo -u $AGENTUSERNM command | echo $AGENTPW 
